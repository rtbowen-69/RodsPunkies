...
function App() {
	...
	const cycleImages = async () => {
		const getRandomNumber = () => {
			const counter = (Math.floor(Math.random() * 249)) + 1
			setCounter(counter)
		}
			...
			<main>
				<section id='welcome' className='welcome'>
					...
					<Row className='header my-3 p-3 mb-0 pb-0'>
						<Col xs={12} md={12} lg={8} xxl={8}>
							<h1>Rods Punkies</h1>
							<p className='sub-header'>Available on 12 / 16 / 23</p>
						</Col>
						<Col className='flex social-icons'>
							<a
								href="https://twitter.com/XZRbowen69"
								target='_blank'
					...
					<Row className='flex m-3'>
						<Col md={5} lg={4} xl={5} xxl={4} className='text-center'>
							<img
								src={`https://ipfs.io/ipfs/QmdwbtHajiyc7i2qSXTep32aJoaE5SEfxEiLDbFUoA9zEd/${counter}.png`}
								alt="Rods Punkies"
								className='showcase'
							/>
						</Col>
						<Col md={5} lg={4} xl={5} xxl={4}>
							...
							<p className='text'>
								By attending the masterclass, I learned how to generate NFT images, upload to IPFS, create your NFT contract, and use OpenSea!
							</p>
				...
				<section id='about' className='about'>
					...
					<Row className='flex m-3'>
						...
						<Col md={5} lg={4} xl={5} xxl={4} className='text-center'>
							<img src={showcase} alt="Multiple Rods Punkies" className='showcase' />
						</Col>
						<Col md={5} lg={4} xl={5} xxl={4}>
							{isError ? (
								<p>{message}</p>
								...
								<div>
									...
									<ul>
										<li>250 generated images using an art generator</li>
